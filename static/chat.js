...
(() => {
  ...
  function _getOrCreateSessionId() {
    let sid = "";
    try { sid = localStorage.getItem(LS_SESSION_KEY) || ""; } catch { /**/ }
    if (!sid || !/^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(sid)) {
      sid = crypto.randomUUID();
      try { localStorage.setItem(LS_SESSION_KEY, sid); } catch { /**/ }
    }
    return sid;
  }
  ...
  async function _saveMessage(role, content, events = [], sources = [], graph = {}) {
    try {
      await fetch(HISTORY_URL, {
        method: "POST",
        headers: _apiHeaders(),
        body: JSON.stringify({ session_id: SESSION_ID, role, content, events, sources, graph }),
      });
    } catch { /**/ }
  }
  ...
  async function _deleteHistory() {
    try {
      await fetch(`${HISTORY_URL}?session_id=${encodeURIComponent(SESSION_ID)}`, {
        method: "DELETE",
  ...
  function clearSidebar() {
    ...
    if (panelSources) panelSources.innerHTML = `<div class="sidebar-hint">Send a message to see retrieved sources.</div>`;
    if (panelGraph)   panelGraph.innerHTML   = `<div class="sidebar-hint">Send a message to see graph context.</div>`;
  }
  ...
  async function _loadSessions() {
    ...
    try {
      ...
      if (sessionsList) sessionsList.innerHTML = `<div class="sessions-loading">—</div>`;
    }
  }
  ...
  async function _deleteSession(sid) {
    ...
    if (sid === SESSION_ID) {
      ...
      try { localStorage.setItem(LS_SESSION_KEY, newSid); } catch { /**/ }
  ...
  function _setActiveMsg(bodyEl) {
    ...
    messagesEl.querySelectorAll(".message-body.ctx-active").forEach(el => el.classList.remove("ctx-active"));
  ...
  async function send() {
    ...
    if (fullAnswer) {
      ...
      if (_pendingThinkingText) {
        _pendingEvents.unshift({ type: "thinking", text: _pendingThinkingText });
      }
      history.push({ role: "assistant", content: fullAnswer });
      await _saveMessage("assistant", fullAnswer, _pendingEvents, _pendingSources, _pendingGraph);
  ...
  function _createAgentTrace() {
    ...
    el.innerHTML =
      `<div class="agent-trace-header">` +
        ...
        `<span class="agent-trace-label">Searching knowledge base…</span>` +
  ...
  function _addAgentThinking(text) {
    ...
    if (!thinkEl) {
      ...
      _currentTrace.stepsEl.insertBefore(thinkEl, _currentTrace.stepsEl.firstChild);
    }
  ...
  function _finalizeAgentTrace(totalDocs) {
    ...
    if (spinner) { spinner.style.animation = "none"; spinner.style.opacity = "0"; }
    const nTools = Object.keys(_currentTrace.stepsMap).length;
    _currentTrace.headerLbl.textContent =
      `Found ${totalDocs} doc${totalDocs !== 1 ? "s" : ""} via ${nTools} tool call${nTools !== 1 ? "s" : ""}`;
    _currentTrace.summaryEl.style.display = "flex";
    _currentTrace.summaryEl.textContent =
      `${nTools} call${nTools !== 1 ? "s" : ""} · ${totalDocs} document${totalDocs !== 1 ? "s" : ""} retrieved`;
    setTimeout(() => {
  ...
  function _renderStaticAgentTrace(events, totalDocs) {
    ...
    const toolResults = {};
    events.filter(e => e.type === "tool_result").forEach(e => { toolResults[e.id] = e; });
    ...
    for (const tc of toolCalls) {
      ...
      try {
        ...
        query = p.query || p.qualified_name || String(Object.values(p)[0] ?? "");
      } catch { /**/ }
      const res = toolResults[tc.id];
      let badgeHtml = "";
      if (res) {
        if (res.error)       badgeHtml = `<span class="agent-step-badge err">error</span>`;
        else if (res.count > 0) badgeHtml = `<span class="agent-step-badge ok">${res.count} doc${res.count !== 1 ? "s" : ""}</span>`;
  ...
  function updateSidebar(docs, graph) {
    ...
    if (sourcesCount) sourcesCount.textContent = docs.length ? String(docs.length) : "";
    if (graphCount)   graphCount.textContent   = hasGraph    ? String(graph.nodes.length) : "";
  ...
  function renderSourcesPanel(docs) {
    return docs.map(doc => {
      ...
      const fileName = doc.file || doc.rel_path || doc.file_path || "unknown";
      return `
        <div class="context-doc">
          <div class="context-doc-header">
            <span class="context-doc-file">${escHtml(fileName)}</span>
  ...
  const LABEL_COLOR = {
    Table:             { background: "#4C8EDA", border: "#3577C4", hover: { background: "#5E9EEC", border: "#3577C4" }, highlight: { background: "#6AAEF5", border: "#3577C4" } },
    Function:          { background: "#D4A843", border: "#B88A28", hover: { background: "#E6BB55", border: "#B88A28" }, highlight: { background: "#F0C060", border: "#B88A28" } },
    Class:             { background: "#6DCE9E", border: "#44A877", hover: { background: "#82DFB0", border: "#44A877" }, highlight: { background: "#8FDFB8", border: "#44A877" } },
    ApiEndpoint:       { background: "#E8735A", border: "#C45030", hover: { background: "#F2876E", border: "#C45030" }, highlight: { background: "#F5957E", border: "#C45030" } },
    CronJob:           { background: "#A78BFA", border: "#7C5BD0", hover: { background: "#BAA3FC", border: "#7C5BD0" }, highlight: { background: "#C4B0FF", border: "#7C5BD0" } },
    Service:           { background: "#F97316", border: "#CA5B08", hover: { background: "#FB8A38", border: "#CA5B08" }, highlight: { background: "#FCA44A", border: "#CA5B08" } },
    Domain:            { background: "#22D3EE", border: "#0A9DB5", hover: { background: "#38DDFA", border: "#0A9DB5" }, highlight: { background: "#67E8F9", border: "#0A9DB5" } },
    Repository:        { background: "#778CA3", border: "#556070", hover: { background: "#8FA0B5", border: "#556070" }, highlight: { background: "#A0B2C4", border: "#556070" } },
    EventTopic:        { background: "#EC4899", border: "#BE185D", hover: { background: "#F472B6", border: "#BE185D" }, highlight: { background: "#F9A8D4", border: "#BE185D" } },
    Module:            { background: "#10B981", border: "#047857", hover: { background: "#34D399", border: "#047857" }, highlight: { background: "#6EE7B7", border: "#047857" } },
    File:              { background: "#64748B", border: "#334155", hover: { background: "#7E8FA3", border: "#334155" }, highlight: { background: "#94A3B8", border: "#334155" } },
    ExternalService:   { background: "#EF4444", border: "#B91C1C", hover: { background: "#F87171", border: "#B91C1C" }, highlight: { background: "#FCA5A5", border: "#B91C1C" } },
    FrontendPage:      { background: "#8B5CF6", border: "#6D28D9", hover: { background: "#A78BFA", border: "#6D28D9" }, highlight: { background: "#C4B5FD", border: "#6D28D9" } },
    FrontendComponent: { background: "#7C3AED", border: "#5B21B6", hover: { background: "#8B5CF6", border: "#5B21B6" }, highlight: { background: "#A78BFA", border: "#5B21B6" } },
    ApiGateway:        { background: "#0EA5E9", border: "#0369A1", hover: { background: "#38BDF8", border: "#0369A1" }, highlight: { background: "#7DD3FC", border: "#0369A1" } },
  };
  const DEFAULT_COLOR = {
    background: "#697A8D", border: "#4A5568",
  ...
  function _mountVisNetwork(graph) {
    ...
    const visNodes = new vis.DataSet(nodes.map((n, i) => {
      ...
      return {
        ...
        font: {
          ...
          bold: { size: 12, face: "Inter, ui-sans-serif, sans-serif", color: "#F1F5F9", mod: "bold" },
        },
        borderWidth:      2,
    ...
    const visEdges = new vis.DataSet(edges.map((e, i) => {
      ...
      const fromId = nodes.find(n => n.props?.qualified_name === e.from)?.props?.qualified_name ?? e.from;
      const toId   = nodes.find(n => n.props?.qualified_name === e.to  )?.props?.qualified_name ?? e.to;
      return {
        id:    "e" + i,
        from:  fromId,
        to:    toId,
        ...
        font: {
          ...
          face:  "Inter, ui-sans-serif, sans-serif",
          align: "middle",
          strokeWidth: 2, strokeColor: "#0D1520",
        },
    ...
    _visNet = new vis.Network(container, { nodes: visNodes, edges: visEdges }, options);
    ...
    _visNet.on("click", params => {
      ...
      const propRows = Object.entries(p)
        .filter(([k, v]) => v != null && v !== "" && v !== false && !skip.has(k))
        .map(([k, v]) => {
          const val = Array.isArray(v) ? v.join(", ") : String(v);
          return `<span class="vis-prop-key">${escHtml(k)}</span>`
      ...
      function _relName(qname) {
        ...
        return peer ? _nodePrimaryName(peer) : (qname || "").split(":").pop() || qname;
      }
      function _relDot(qname) {
        const peer = _nodeById[qname];
        const c = peer ? (LABEL_COLOR[peer.label] || DEFAULT_COLOR).background : "#64748B";
      ...
      detail.innerHTML =
        `<div class="vis-node-detail-header">
    ...
    if (legend) {
      ...
      legend.innerHTML = labels.map(lbl => {
  ...
  // ── Utilities ─────────────────────────────────────────────────────────────
  function setBusy(val) { busy = val; textarea.disabled = val; sendBtn.disabled = val; }
